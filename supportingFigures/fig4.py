...
Usage:
    python3 fig4.py
    python3 fig4.py --base_dir /path/to/bootstrap_uncertainty --geojson /path/to/gadm41_DNK_1.json
    python3 fig4.py --cnn_dir /path/to/cblv-cnn_output
"""
...
# ---------------------------------------------------------------------------
class _Tee:
    """Mirror writes across multiple streams (used to tee stdout to fig4.out)."""
...
def report_bootstrap_coverage(clade_data, labels, metrics):
    ...
    whole grid at once, written to fig4.out so the figure and the number quoted
...
def main():
    """Build fig4.pdf — 5-clade trees + ML SSS / R_0 choropleths + bootstrap violins.
