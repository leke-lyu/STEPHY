...
Panel (c) — INFERENCE
    ...
    predicted index location + 95% CP set.
...
def _draw_panel_a(fig, subplotspec):
    ...
    ax_a1.text(2.5, 5.0, 'Loc$_a$, index location',
...
# Predicted index location (classification answer) and its CP set at 95%.
...
def _draw_pred_card(ax, x_center, y_center, loc, is_target):
    ...
    index location is communicated by the star + readout above, not by
    card highlighting."""
    p = PANEL_F_PREDS[loc]
...
def _draw_panel_f_predictions(ax):
    ...
    card. Predicted index location (Loc_a) gets a gold border + gold star
    ...
    # Single-line classification readout: ★ + index-location prediction +
    # CP set inline. Font / color match the panel-(a) caption
    # ("Loc_a, index location") so the question-answer pair reads
    ...
    ax.text(star_x + 0.45, star_y,
            f'index location: Loc$_{{{PANEL_F_PRED_INDEX}}}$  '
            f'[{cp_set_str}]',
            ha='left', va='center',
            fontsize=FS_PANEL_A_QUES, fontweight='bold', style='italic',
