#!/usr/bin/env python3
"""
denmark_sample.py — Denmark 2021 weekly epidemic summary (4 rows x 3 wave
columns). Supplementary figure documenting how the sequences behind the
Denmark case study were sampled.
...
sequences fall inside its window. Per-column coverage is printed to
denmark_sample.out.

Row d is capped at 15%; an open caret above the cap marks a series that runs
off-scale there, and every such point is listed in denmark_sample.out.

Usage:
    python3 denmark_sample.py
    python3 denmark_sample.py --data_root /path/to/sars_cov2_290k_denmark-main \
...
class _Tee:
    """Mirror writes across multiple streams (tees stdout to denmark_sample.out)."""
...
def main():
    parser = argparse.ArgumentParser(
        description='denmark_sample: Denmark 2021 weekly epidemic summary.')
    parser.add_argument(
        '--data_root', type=str,
    ...
    log = open(out_dir / 'denmark_sample.out', 'w')
    ...
    out_pdf = out_dir / 'denmark_sample.pdf'
    out_png = out_dir / 'denmark_sample.png'
