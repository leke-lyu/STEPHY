...
def create_species_name(pop, idx):
    """
    Create standardized species name from population type and index.

    Args:
        pop: Population type ('S', 'I', 'R', 'sample')
        idx: Location index (or NaN for R/sample)

    Returns:
        Formatted species name (e.g., 'S[0]', 'I[5]', 'R', 'sample')

    Examples:
        >>> create_species_name('S', 0)
        'S[0]'
        >>> create_species_name('R', None)
        'R'
    """
...
def load_trajectory_wide(traj_file):
    ...
    # Create standardized species names
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 2:
        ...
        df = load_trajectory_wide(traj_file)
        print(f"Loaded {len(df)} rows, {len(df.columns)} columns")
        print(f"\nColumns: {list(df.columns)}")
    else:
        print("Usage: python3 trajectory_utils.py <trajectory_file>")
