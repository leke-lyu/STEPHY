...
- Label extraction (R0, Source_Sink_Score)
"""
...
class VirtualSubtreeEncoder:
    ...
    CBLV Matrix Structure (subtree_width × 4):
    ...
    def encode_cblv(self, loc, subtree_width=None, rescale=True):
        ...
        Args:
            ...
            subtree_width: Pad to this width (None = no padding)
            rescale: Rescale by tree height

        Returns:
            heights: (subtree_width, 4) matrix of CBLV features
...
def extract_labels(input_folder, file_prefix, tree_idx, num_nodes):
    ...
    try:
        ...
        print(f"Warning: Could not compute Source_Sink_Score for {file_prefix}: {e}")
        sss = np.full(num_nodes, np.nan, dtype=np.float32)
...
def normalize_edge_features(graphs):
    ...
    return graphs, {'mean': mean, 'std': std}
