#!/usr/bin/env python3
"""
Performance scatter — STEPHY per-target model diagnostics (single row).

Extracted from fig2.py Row 1: for each target, STEPHY's predictions on the
test split are shown as a true-vs-predicted scatter (regression) or a
per-state accuracy plot (classification).
...
def main():
    """
    Build the performance-scatter figure — STEPHY's per-target diagnostics on
    the test split (regression scatter / per-state classification accuracy),
    one panel per target in a single row.
