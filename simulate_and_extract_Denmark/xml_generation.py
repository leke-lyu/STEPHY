...
CONFIG = {
    ...
    'fixed_pop_sizes': list(map(int, get_required_env('FIXED_POP_SIZES').split())),
    ...
    'R0_distribution': get_required_env('R0_DISTRIBUTION'),
    'R0_alpha': float(get_required_env('R0_ALPHA')),
    'R0_beta': float(get_required_env('R0_BETA')),
...
def generate_constrained_values(num_locs, value_range, max_diff,
    ...
    if distribution == 'beta':
        # Draw from Beta and scale to the feasible range
        raw = np.random.beta(alpha, beta_param)
        anchor = feasible_min + raw * (feasible_max - feasible_min)
    else:
        anchor = np.random.uniform(feasible_min, feasible_max)

    # Generate values within max_diff of the anchor (uniform spread)
...
def R0_values(num_locs, R0_range, shared=False, max_diff=None,
    ...
    if shared:
        if distribution == 'beta':
            raw = np.random.beta(alpha, beta_param)
            single_R0 = R0_range[0] + raw * (R0_range[1] - R0_range[0])
        else:
            single_R0 = np.random.uniform(R0_range[0], R0_range[1])
        R0_array = np.full(num_locs, single_R0)
    else:
        R0_array = generate_constrained_values(
            num_locs, R0_range, max_diff,
            distribution=distribution, alpha=alpha, beta_param=beta_param)
...
def generate_parameters(config):
    ...
    # Fixed population sizes for Danish regions
    pop_sizes = np.array(config['fixed_pop_sizes'])
    ...
    R0_array = R0_values(
        ...
        distribution=config['R0_distribution'],
        alpha=config['R0_alpha'], beta_param=config['R0_beta'],
    )
