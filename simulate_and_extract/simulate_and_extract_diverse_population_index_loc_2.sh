...
# Multi-seed introduction: this many distinct random locations each receive one
# infected at t = 0, introduced from an unsampled origin X so the tree stays
# single-rooted (see xml_generation.generate_xml). SEED_LOCATION must be "None".
NUM_SEED_LOCATIONS=2
...
# Sampling rate (δ, shared across all locations)
DELTA_MIN=0.0002
DELTA_MAX=0.0028
...
export NUM_LOCS SEED_LOCATION NUM_SEED_LOCATIONS RANDOM_SEED TIME_UNITS NUM_SIMS
