...
# Sampling rate (δ, heterogeneous across time, shared across locations)
# A baseline x is drawn per outbreak; δ then steps through three phases over
# equal thirds of the simulation time: x*(1-h), x, x*(1+h) with h =
# DELTA_HETERO_TIME, i.e. 0.6x -> 1x -> 1.4x. β follows δ phase by phase so R0
# stays exact. The baseline range is shrunk to [DELTA_MIN/(1-h), DELTA_MAX/(1+h)]
# so every phase's δ stays inside the training range [DELTA_MIN, DELTA_MAX].
DELTA_MIN=0.0002
DELTA_MAX=0.0028
DELTA_HETERO_TIME=0.4
...
export DELTA_MIN DELTA_MAX DELTA_HETERO_TIME
