#!/usr/bin/env python3
"""
Training script for STEPHY (Phylogeny-only Model + Aux Branch).
...
def parse_args():
    parser = argparse.ArgumentParser(description='STEPHY: Train with phylogeny + aux data')
...
def normalize_edge_features(train_graphs, *other_graph_lists):
...
def _forward_batch(model, batched_g, label_name):
    ...
    For standard model: flat (N,) -> per_graph (batch, num_locations)
    For CQR model: flat (N, 3) -> per_graph (batch, num_locations, 3)
    """
...
def train_epoch(model, dataloader, optimizer, criterion, label_name, is_classification=False):
    ...
    graphs have the same N. For CQR, PinballLoss takes (batch, N, 3) preds
    and (batch, N) labels.
    """
    model.train()
    total_loss = 0
...
def evaluate(model, dataloader, criterion, label_name, is_classification=False):
    ...
    For regression: preds/labels are per-node scalar values (or (N,3) for CQR).
    """
    model.eval()
    total_loss = 0
    total_graphs = 0
...
def main():
    ...
    edge_norm = normalize_edge_features(train_graphs, *other_sets)
    ...
    if use_cp:
        ...
        if is_classification:
            ...
            # Apply to test set (returns true-scale results)
            ...
            # Standard regression metrics on point predictions
